...
import isoTimeFormat from "../lib/iosTimeFormat"
    ...
    <div key={row} className="flex gap-2 mt-2">
      <div className="flex flex-wrap items-center justify-center gap-2">
        {Array.from({ length: count }, (_, i) => {
          ...
          return (
            <button key={seatId} onClick={() => handleSeatClick(seatId)} className={`h-8 w-8 rounded border border-primary/60 cursor-pointer ${selectedSeats.includes(seatId) &&
      ...
      <div className='w-60 bg-primary/10 border border-primary/20 rounded-lg py-10 h-max md:sticky md:top-30'>
        ...
        <div className="mt-5 space-y-1">
          {show.dateTime[date].map((item) => (
            ...
            rounded-r-md cursor-pointer transition ${selectedTime?.time === item.time ?
